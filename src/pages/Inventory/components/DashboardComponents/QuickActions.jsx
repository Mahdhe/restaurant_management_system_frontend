  ...
  return (
    <section className="rounded-[14px] border border-white/15 bg-[#1c2a38]">
      ...
      <div className="border-b border-white/15 px-5 pt-5 pb-2.5">
        <h2 className="text-[18px] font-bold text-[#f0f4f8]">Quick Actions</h2>
      </div>
      ...
      <div className="space-y-2 py-2 px-5">
        {Buttons.map((button) => (
          <button
            key={button.id}
            className={`h-9 w-full rounded-[10px] text-[14px] font-semibold border ${button.borderColor} ${button.buttonColor} ${button.textColor}`}
          >
            {button.label}
          </button>
        ))}
