export default function QueueOrderCard({
  orderNo,
  table,
  waiter,
  status,
  ...
  return (
    <div className="rounded-2xl border border-white/15 px-3 sm:px-4 bg-[#1c2a38] font-dmsans pb-4 min-w-0">
        ...
        {items.map((item, index) => (
          ...
          >
            ...
            <div className="flex items-center justify-between gap-2">
              <p className="text-[13px] sm:text-[14px] font-bold text-white leading-4.5 min-w-0">
                {item}
              </p>

              <span className="text-[15px] sm:text-[16px] font-bold leading-4.5 text-center items-center text-[#e67e22] shrink-0">
                {itemsQuantity[index]}
              </span>
            </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <button
          className={`mt-6 w-full rounded-[10px] items-center text-xs sm:text-sm font-semibold border ${buttonBorder} ${buttonTextColor} ${buttonColor} py-2 px-2 sm:px-4`}
        >
          {buttonText}
        </button>

        <button className="mt-6 rounded-[10px] items-center text-xs sm:text-sm font-semibold py-2 px-2 sm:px-4 bg-[#243447] border border-white/15">
          View
        </button>
      </div>
    </div>
