...
export default function RevenueSummary() {
  return (
    <div className="mt-4 rounded-[14px] border border-white/15 bg-[#1c2a38] overflow-hidden pb-5">
      {/* header */}
      ...
      <div className="px-5 py-5">
        {data.map((item) => (
          <div key={item.label} className="border-b border-white/10 py-3">
            <p className="text-[13px] uppercase text-[#8a9bb0]">{item.label}</p>

            <p className={`text-[24px] font-bold ${item.valueColor}`}>
              {item.value}
            </p>
          </div>
        ))}
